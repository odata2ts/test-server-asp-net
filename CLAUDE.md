...
cut by release-please. The image a release ships is built and smoke-tested while its release PR is open,
pushed as `:rc`, and merging only re-tags that manifest as `0.2.1`, `0.2`, `0` and `latest` — so
publishing takes seconds and ships exactly what was tested, and `publish-image.yml` never builds twice.
Automated consumers pin an exact version — `odata2ts/int-test/asp-net` does, and the release dispatches
there so the PR raising that pin opens at once. Locally
