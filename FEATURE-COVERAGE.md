...
| `$batch` reference nothing resolves                 | ✅ | ✔ | ✔ | an unresolvable first-segment reference without a `dependsOn` covering it is refused at parse, before any request is answered: the whole batch fails with a 400 carrying the parser's message - no per-slot error is possible, and the 400 is the handler's, because the parser answers ODataException, the catch-all the batch writer shares |
...
| `Core.Computed` (`Medium/PopularityScore`, nine keys) | ✅ | ✔ | ✔ | emitted **and** enforced: a value sent on insert or update is ignored, per Protocol 11.4.3 |
...
**Every key is annotated except two.** Nothing in CSDL says whether a key is supplied by the client or
generated by the server, so the reference model leaves that to each implementation - and here the database
owns all of them but two, which therefore carry `Core.Computed` and behave like any other computed
property: a value sent on create is discarded.

The two exceptions are the point of the exercise. `Branch/Id` is a code the organisation allocates and
`Copy`'s composite key is picked by whoever shelves the copy, so both stay bare and both are creatable -
`POST /Branches` exists for no other reason. A client generated from this metadata can therefore tell a
key it must supply from one it must not, which no other server-side signal gives it.
